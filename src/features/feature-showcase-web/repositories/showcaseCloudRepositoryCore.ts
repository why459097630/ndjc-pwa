...
import {
  ...
  LOCAL_DEVELOPMENT_STORE_ID,
  canUseDevelopmentDefaultStoreId,
  normalizeStoreId,
  ensureLeadingSlash,
  trimSlashes,
  normalizePathPart,
  ...
  jsonBoolean,
  parseStringArray,
  parseCloudImageVariants,
  normalizePushNotificationImageUrl,
  pickCloudStorePwaPushIconUrl,
  pickCloudStorePwaAppName,
  pushBadgeUrlForPushType,
  isCloudImageVariantSchemaError,
  stripCloudImageVariantColumns,
  parseIsoMillis,
  formatIsoUtcMillis,
  buildRawArrayBody,
  isRawArrayPayload,
  stringifyRequestBody,
  buildCategoryWriteErrorMessage,
  decodeJwtPayload,
  tokenExpiresAt
} from './showcaseRepositoryCommon'
...
async fetchCategories(storeIdInput?: string | null): Promise<CloudCategory[]> {
  ...
  const query = [
    'select=id,store_id,name,name_i18n,sort_order,created_at',
    `store_id=${this.encodeEq(storeId)}`,
    'order=sort_order.asc.nullslast,created_at.asc.nullslast,id.asc'
  ].join('&')
  ...
  async fetchDishesFilteredPage(input: {
    ...
    return this.executeOnce('fetchDishesFilteredPage', async () => {
      ...
      const sortedRows = filteredRows.slice().sort((left, right) => {
        ...
        return jsonString(leftRow, 'id').localeCompare(jsonString(rightRow, 'id'))
      })
...
async ensureCategoryExists(storeIdInput: string | null | undefined, nameInput: string): Promise<CategoryWriteResult> {
  ...
  const nextSortOrder = categories.reduce((max, item) => {
...
async resolveOrCreateCategoryId(storeIdInput: string | null | undefined, categoryNameInput: string | null | undefined): Promise<string | null> {
  ...
  const nextSortOrder = existing.reduce((max, item) => {
    const sortOrder = Number(item.sortOrder)
...
async renameCategoryById(input: {
  ...
  async fetchAppointmentRequests(input: {
    ...
    const cancelledByNot = String(input.cancelledByNot || '').trim()
    ...
    const query = [
      ...
      cancelledByNot ? `or=${this.urlEncode(`(cancelled_by.is.null,cancelled_by.neq.${cancelledByNot})`)}` : '',
